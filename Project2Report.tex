...
\subsubsection{Random Dots and Boxes}
\paragraph{When I was having trouble implementing more complex algorithms, I created a couple of basic functions to place random yellow pixels and yellow boxes in the middle of a grass image and white pixels and boxes in dandelion images. Surprisingly, the dandelion images were fairly easy to fool with random white dots, but the grass images became even more sure they were grass with the yellow pixels and yellow boxes. Ultimately, I decided to stick with trying to implement a JSMA function because the random boxes were unable to fool the grass images at all.}
...
Since we were unable to get all of our sub-algorithms functioning, we did not get to debug the main algorithm. However, we have listed below a few problems we expect would have been challenging in addition to a few challenges we faced in designing the main algorithm.
\newline
...
\paragraph{Sciforce. (2022, September 7). Adversarial attacks explained (and how to defend ML models against them). Medium. Retrieved April 28, 2023, from https://medium.com/sciforce/adversarial-attacks-explained-and-how-to-defend-ml-models-against-them-d76f7d013b18.}
